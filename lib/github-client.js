...
async function bootstrapEmptyRepo(token, owner, repo, branch, seedPath, seedContentUtf8) {
  const r = await ghJson(token, "PUT", `/repos/${owner}/${repo}/contents/${encodePath(seedPath)}`, {
...
export async function pushAtomic(token, { owner, repo, branch, files, commitMessage, authorName, authorEmail }) {
  ...
  if (parentSha === null) {
    // Pick a seed file: the smallest one we have, ideally the marker.
    let seed = files.find((f) => f.path === ".ghl-aistudio-sync.json");
    if (!seed) {
      // Fall back to the smallest file.
      seed = files.slice().sort((a, b) => (a.content?.length || 0) - (b.content?.length || 0))[0];
    }
    parentSha = await bootstrapEmptyRepo(token, owner, repo, branch, seed.path, seed.content);
    bootstrapped = true;
  }

  // 1. Create blobs concurrently with a small pool.
  const blobShas = await mapConcurrent(files, BLOB_CONCURRENCY, async (f) => {
